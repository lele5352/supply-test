...
class TestTransfer(object):
    ...
    def test_transfer_flow(self, goods_list, trans_out_id, trans_out_to_id, trans_in_id, trans_in_to_id, expected, bak):
        with allure.step("获取调出仓上架库位"):
            get_out_sj_kw_ids_result = wms_app_robot.get_kw(1, 5, len(goods_list), trans_out_id, trans_out_to_id)
        ...
        with allure.step("生成调拨需求"):
            # 生成调拨需求
            trans_out_code = wms_app_robot.ck_id_to_code(trans_out_id)
            trans_out_to_code = wms_app_robot.ck_id_to_code(trans_out_to_id)
            trans_in_code = wms_app_robot.ck_id_to_code(trans_in_id)
            trans_in_to_code = wms_app_robot.ck_id_to_code(trans_in_to_id)
            demand_list = wms_transfer_service_robot.get_demand_list(goods_list, trans_out_code, trans_out_to_code,
                                                                     trans_in_code, trans_in_to_code)
            assert demand_list is not None

        with allure.step("创建调拨拣货单"):
            # 创建调拨拣货单
            create_pick_order_result = wms_app_robot.transfer_out_create_pick_order(demand_list, 1)
        ...
        with allure.step("获取调拨出库托盘库位"):
            get_trans_out_tp_kw_ids_result = wms_app_robot.get_kw(1, 3, len(pick_sku_list), trans_out_id,
                                                                  trans_out_to_id)
