...
class TestOtterInStock(object):
    ...
    @pytest.mark.parametrize("ware_sku_qty_list,expected", data)
    def test_up_shelf_one_single_goods(self, ware_sku_qty_list, expected):
        """
        ...
        cp_location_ids = [wms_app_robot.get_kw(1, 6, len(ware_sku_qty_list), self.warehouse_id, self.to_warehouse_id)]

        # ims_robot.dbo.delete_qualified_inventory(sale_skus)
        # ims_robot.dbo.delete_unqualified_inventory(sale_skus)

        res = ims_robot.cp_other_in(ware_sku_qty_list, cp_location_ids, self.warehouse_id, self.to_warehouse_id)
        assert res['code'] == 200

        for sale_sku in sale_skus:
            unqualified_inventory = ims_robot.dbo.query_format_cp_inventory(sale_sku, self.warehouse_id)
            ...
            qualified_inventory = ims_robot.query_lp_inventory(sale_sku, self.warehouse_id, self.to_warehouse_id)
    ...
    def test_2_up_shelf_many_single_goods_to_one_location(self):
        ...
        for sale_sku in sale_skus:
            ...
            expect_cp_inventory = ims_logics.get_cp_other_in_expect_inventory(
                ware_sku_qty_list, cp_location_ids).get(sale_sku)
            assert expect_cp_inventory == unqualified_inventory
