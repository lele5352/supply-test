...
def run_receive(distribute_order_code, warehouse_id, to_warehouse_id):
    ...
    search_result = wms_app.entry_order_page([distribute_order_code])
    ...
    scan_result = wms_app.entry_order_detail(entry_order_code)
    ...
    get_sh_kw_result = wms_app.get_kw(2, 1, len(receive_sku_list), warehouse_id, to_warehouse_id)
    if not get_sh_kw_result['code']:
        return 'Fail', None

    sh_kw_codes = get_sh_kw_result['data']

    get_sj_kw_result = wms_app.get_kw(2, 5, len(receive_sku_list), warehouse_id, to_warehouse_id)
    ...
    receive_result = wms_app.confirm_receive(entry_order_code, pre_receive_order_code, receive_sku_list)
    if not receive_result['code']:
        return 'Fail', None

    pre_receive_order_list = [pre_receive_order_code]

    submit_pre_receive_order_result = wms_app.submit_pre_receive_order(pre_receive_order_list)
    if not submit_pre_receive_order_result['code']:
        return 'Fail', None

    # 跳过质检，直接上架交接
    handover_result = wms_app.handover_to_upshelf(sh_kw_codes)
    if not handover_result['code']:
        return 'Fail', None

    # 整托上架
    for sh_kw_code, sj_kw_code in zip(sh_kw_codes, sj_kw_codes):
        kw_detail_result = wms_app.location_detail(sh_kw_code)
        if not kw_detail_result['code']:
            return 'Fail', None

        upshelf_result = wms_app.upshelf_whole_location(sh_kw_code, sj_kw_code)
        if not upshelf_result['code']:
            return 'Fail', None

    # 最后需要再调用上架完成接口，结束流程
    complete_upshelf_result = wms_app.complete_upshelf()
