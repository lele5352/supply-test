...
def run_front_label_delivery(delivery_order_info, delivery_order_detail, flow_flag=None):
    ...
    if not wms_app.is_success(assign_stock_result) or assign_stock_result["data"]["failNum"] > 0:
    ...
    if package_state != 2:
        ...
        package_call_back_result = wms_app.delivery_mock_package_call_back(delivery_order_code, transport_mode,
                                                                           order_sku_list)
        if not wms_app.is_success(package_call_back_result):
    ...
    until(99, 0.5)(lambda: wms_app.dbo.query_delivery_order_package_info(delivery_order_code) is not None)()
    ...
    if express_state != 2:
        ...
        if not wms_app.is_success(label_call_back_result):
    ...
    if not wms_app.is_success(review_result) or review_result["data"]["failSize"] > 0:
    ...
    if not wms_app.is_success(shipping_result):
...
def run_backend_label_delivery(delivery_order_info, delivery_order_detail, flow_flag=None):
    ...
    if not wms_app.is_success(package_info_result):
    ...
    if not wms_app.is_success(save_package_result):
...
def run_delivery(delivery_order_code, warehouse_id, flow_flag=None):
    ...
    if not wms_app.is_success(delivery_order_page_result) or wms_app.is_data_empty(delivery_order_page_result):
    ...
    if not wms_app.is_success(delivery_order_detail_result):
...
if __name__ == "__main__":
    delivery_order_code = "PRE-CK2306290004"
    warehouse_id = 513
