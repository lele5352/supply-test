...
class WmsTransferDataGenerator:
    ...
    def create_transfer_demand(self, trans_out_id, trans_out_to_id, trans_in_id, trans_in_to_id, sale_sku_code,
        ...
        if not central_inventory or central_inventory['central_inventory_sale_block'] >= central_inventory[
            ...
            add_stock_res = self.ims.add_stock_by_other_in(
                ...
                'A',
                trans_qty,
                self.wms.db_get_kw(1, 5, 1, trans_out_id, trans_out_to_id),
                trans_out_id,
                trans_out_to_id)
    ...
    def create_transfer_pick_order(self, trans_out_id, trans_out_to_id, trans_in_id, trans_in_to_id, sale_sku_code,
        ...
        create_pick_order_res = self.wms.transfer_out_create_pick_order([demand_no], 1)
        if not create_pick_order_res:
            log.error('创建调拨拣货单失败！')
            return
        barcode_generate(create_pick_order_res['data'], 'transfer_pick_order')
        print('调拨拣货单号：%s' % create_pick_order_res['data'])
        return create_pick_order_res['data']
    ...
    def create_transfer_out_order(self, trans_out_id, trans_out_to_id, trans_in_id, trans_in_to_id, sale_sku_code,
        ...
        # 创建调拨拣货单
        pick_order_res = self.wms.transfer_out_create_pick_order([demand_no], 1)
        ...
        # 分配调拨拣货人
        assign_pick_user_res = self.wms.transfer_out_pick_order_assign([pick_order_code], 'admin', 1)
        if not assign_pick_user_res:
            log.error('创建调拨出库单失败：分配拣货人异常！')
            return
        # 获取调拨拣货单详情数据
        pick_order_details = self.wms.transfer_out_pick_order_detail(pick_order_code)
        if not pick_order_details:
            log.error('创建调拨出库单失败：获取拣货单详情数据异常！')
            return
        # 调拨拣货单确认拣货-纸质
        confirm_pick_res = self.wms.transfer_out_confirm_pick(pick_order_details)
        if not confirm_pick_res:
            log.error('创建调拨出库单失败：确认拣货异常！')
            return
        # 调拨拣货单按需装托提交
        trans_out_tp_kw_ids = self.wms.db_get_kw(1, 3, demand_qty, trans_out_id, trans_out_to_id)
        submit_tray_res = self.wms.transfer_out_submit_tray(pick_order_details, trans_out_tp_kw_ids)
        if not submit_tray_res:
            log.error('创建调拨出库单失败：装托完成异常！')
            return
        # 查看整单获取已装托的托盘
        tray_detail_res = self.wms.transfer_out_pick_order_tray_detail(pick_order_code)
        ...
        finish_packing_res = self.wms.transfer_out_finish_packing(pick_order_code, tray_list)
        ...
        transfer_out_order_detail_res = self.wms.transfer_out_order_detail(transfer_out_order_no)
        ...
        for box_no, tray_code in details:
            ...
            print('生成箱单：%s' % box_no)
            review_res = self.wms.transfer_out_order_review(box_no, tray_code)
            if not review_res:
                log.error('创建调拨出库单失败：箱单复核异常！')
                return
        # 调拨发货绑定交接单和箱单
        for detail in details:
            bind_res = self.wms.transfer_out_box_bind(detail[0], '', '')  # 交接单号和收货仓编码实际可以不用传
            if not bind_res:
                log.error('创建调拨出库单失败：调拨发货交接单绑定箱单失败！')
                return
            handover_no = bind_res['data']['handoverNo']

        delivery_res = self.wms.transfer_out_delivery(handover_no)
        if not delivery_res:
