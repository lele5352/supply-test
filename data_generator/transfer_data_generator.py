...
class WmsTransferDataGenerator:
    ...
    @staticmethod
    def generate_cds_cabin_order(sku_list, step=12, container_no=None, so_number=None, trans_out_id=542, trans_in_id=539):
        """
        生成退税海柜单
        :param list[list[str]] sku_list: 需要发柜的sku
        ...
        for i in range(0, len(sku_list), step if step <= 15 else 15):
            ...
            order = transfer_data.create_cabinet_order(
                trans_out_id, trans_out_id, trans_in_id, trans_in_id, sec_sku_list, "A", 1)
            container_no, so_number = transfer_data.submit_cabinet_order(order, container_no, so_number)
...
if __name__ == '__main__':
    ...
    transfer_data.generate_cds_cabin_order(sku_list=multi_sku_list, step=10)
