...
class WmsTransferDataGenerator:
    ...
    def create_transfer_demand(self, trans_out_id, trans_out_to_id, trans_in_id, trans_in_to_id, sale_sku_code,
        ...
        if not central_inventory or central_inventory['central_block'] >= central_inventory[
            ...
            add_stock_res = self.ims_logics.add_qualified_stock_by_other_in(
                ...
                'A',
                trans_qty,
                self.wms_request.db_get_kw(1, 5, 1, trans_out_id, trans_out_to_id),
                trans_out_id,
                trans_out_to_id)
    ...
    def create_transfer_out_order(self, trans_out_id, trans_out_to_id, trans_in_id, trans_in_to_id, sale_sku_code,
        ...
        if not assign_pick_user_res:
            ...
            return
        # 获取调拨拣货单详情数据
        pick_order_details = self.wms_request.transfer_out_pick_order_detail(pick_order_code)
        if not pick_order_details:
            log.error('创建调拨出库单失败：获取拣货单详情数据异常！')
            return
        # 调拨拣货单确认拣货-纸质
        confirm_pick_res = self.wms_request.transfer_out_confirm_pick(pick_order_details)
        if not confirm_pick_res:
            log.error('创建调拨出库单失败：确认拣货异常！')
            return
        # 调拨拣货单按需装托提交
        trans_out_tp_kw_ids = self.wms_request.db_get_kw(1, 3, demand_qty, trans_out_id, trans_out_to_id)
