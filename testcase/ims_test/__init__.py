...
fsj_location_ids = [ims.wms_api.get_location_id(location_code, delivery_warehouse_id) for location_code in
                    fsj_location_codes]
# 中转仓上架库位
zsj_location_codes = ims.wms_api.get_location_codes(len(bom_detail), 5, exchange_warehouse_id)
...
fcp_location_id = ims.wms_api.get_location_id(fcp_location_code, delivery_warehouse_id)

# 中转仓次品库位
zcp_location_code = ims.wms_api.get_location_codes(1, 6, exchange_warehouse_id)
zcp_location_id = ims.wms_api.get_location_id(fcp_location_code, exchange_warehouse_id)

# 备货仓次品库位
bcp_location_code = ims.wms_api.get_location_codes(1, 6, stock_warehouse_id)
bcp_location_id = ims.wms_api.get_location_id(fcp_location_code, stock_warehouse_id)

# 发货仓移库库位
yk_location_code = ims.wms_api.get_location_codes(1, 4, delivery_warehouse_id)
yk_location_id = ims.wms_api.get_location_id(yk_location_code, delivery_warehouse_id)
