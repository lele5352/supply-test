...
class TestQualifiedGoodsOtherIntoDeliveryWarehouse(object):
    ...
    def test_5_up_shelf_many_multiple_goods_to_one_location(self):
        ...
        IMSDBOperator.delete_qualified_inventory(sale_skus)
        IMSDBOperator.delete_unqualified_inventory(sale_skus)
        res = ims_request.qualified_goods_other_in(ware_sku_qty_list, sj_location_ids, self.warehouse_id, self.to_warehouse_id)
        assert res['code'] == 200
    ...
    def test_9_up_shelf_multiple_goods_less_than_one_set_to_one_location(self):
        ...
        for sale_sku in sale_skus:
            expect_qualified_inventory = ims_logics.get_other_in_expect_inventory(ware_sku_qty_list, sj_location_ids).get(sale_sku)
