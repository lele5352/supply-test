...
class TestUnqualifiedGoodsOtherOut(object):
    ...
    def test_1_unqualified_goods_other_out_from_one_location(self):
        ...
        ims_request.unqualified_goods_other_in(ware_sku_qty_list, cp_kw_ids, self.warehouse_id, self.to_warehouse_id)
        ...
        for (ware_sku, qty), cp_kw_id in zip(ware_sku_qty_list, cp_kw_ids):
            ...
            block_res = ims_request.unqualified_goods_other_out_block(sub_ware_sku_qty_list, sub_cp_kw_ids, self.warehouse_id)
            ...
            delivered_res = ims_request.unqualified_goods_other_out_delivered(sub_ware_sku_qty_list, sub_cp_kw_ids,
                                                                              self.warehouse_id)
            self.expect_inventory[ware_sku][cp_kw_id]['block'] -= qty
            self.expect_inventory[ware_sku]['total']['block'] -= qty
            ...
            after_delivered_inventory = IMSDBOperator.query_unqualified_inventory(sale_sku, self.warehouse_id)
    ...
    def test_3_unqualified_goods_other_out_from_several_locations(self):
        ...
        block_res = ims_request.unqualified_goods_other_out_block(ware_sku_qty_list, cp_kw_ids, self.warehouse_id)
        for (ware_sku, qty), cp_kw_id in zip(ware_sku_qty_list, cp_kw_ids):
            self.expect_inventory[ware_sku][cp_kw_id]['block'] += qty
            self.expect_inventory[ware_sku]['total']['block'] += qty
        ...
        delivered_res = ims_request.unqualified_goods_other_out_delivered(ware_sku_qty_list, cp_kw_ids, self.warehouse_id)
        for (ware_sku, qty), cp_kw_id in zip(ware_sku_qty_list, cp_kw_ids):
            self.expect_inventory[ware_sku][cp_kw_id]['block'] -= qty
            self.expect_inventory[ware_sku]['total']['block'] -= qty
            self.expect_inventory[ware_sku][cp_kw_id]['stock'] -= qty
