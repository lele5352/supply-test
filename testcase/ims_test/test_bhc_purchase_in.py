...
class TestBHCPurchaseIn(object):
    def setup_class(self):
        ...
        self.sj_kw_ids = wms_logics.base_get_kw(1, 5, len(self.ware_sku_qty_list), self.warehouse_id, self.to_warehouse_id)
    ...
    def test_1_purchase_create_order(self):
        ...
        for sale_sku in self.sale_sku_suite_dict.keys():
            ...
            expect_lp_inventory = ims_logics.get_purchase_create_order_expect_inventory(
                self.ware_sku_qty_list).get(sale_sku)
            assert res['code'] == 200
