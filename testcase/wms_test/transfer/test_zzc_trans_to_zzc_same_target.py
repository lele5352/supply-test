...
class TestZZCTransToZZCSameTarget:
    def setup_class(self):
        # 设定调出仓为中转仓1，调入仓为中转仓2,目的仓相同
        self.trans_out_id = exchange_warehouse_id  # 调出仓id
        self.trans_out_to_id = delivery_warehouse_id  # 调出仓的目的仓id
        self.trans_in_id = exchange_warehouse_id2  # 调入仓id
        self.trans_in_to_id = delivery_warehouse_id  # 调入仓的目的仓id

    def setup(self):
        wms_request.common_switch_warehouse(self.trans_out_id)
    ...
    def test_1_trans_multiple_goods_components_break_up_suite(self):
        ...
        trans_out_sj_kw_ids = wms_logics.base_get_kw(1, 5, len(origin_inventory), self.trans_out_id, self.trans_out_to_id)
        ...
        if len(pick_sku_list) > 1:
            trans_out_tp_kw_ids = wms_logics.base_get_kw(1, 3, len(pick_sku_list), self.trans_out_id,
                                                         self.trans_out_to_id)
        else:
            trans_out_tp_kw_ids = [
                wms_logics.base_get_kw(1, 3, len(pick_sku_list), self.trans_out_id, self.trans_out_to_id)]
        ...
        # 切换仓库到调入仓
        wms_request.common_switch_warehouse(self.trans_in_id)
        if len(pick_sku_list) > 1:
            trans_in_sj_kw_ids = wms_logics.base_get_kw(1, 5, len(pick_sku_list), self.trans_in_id, self.trans_in_to_id)
        else:
            trans_in_sj_kw_ids = [wms_logics.base_get_kw(1, 5, len(pick_sku_list), self.trans_in_id, self.trans_in_to_id)]
        trans_in_sj_kw_codes = [wms_logics.db_kw_id_to_code(kw_id) for kw_id in trans_in_sj_kw_ids]
        # 调拨入库收货
        ...
        # --------------------------------调入仓库存更新--------------------------------#
        # 调出仓和调入仓目的仓相同，则中央库存不变；goods_inventory表插入为0数据，因为不成套
    ...
    def test_4_trans_multiple_goods_one_set_components(self):
        ...
        # --------------------------------调入仓库存更新--------------------------------#
        # 调出仓和调入仓目的仓相同，则中央库存不变；goods_inventory表现货的stock和remain对应增加，调拨在途转现货，故写入0
        expect_trans_in_inventory.update({
            # "central_stock": 1,
            # "central_block": 0,
            # "central_remain": 1,
    ...
    def test_9_trans_multiple_goods_three_suites_with_two_bom_stock(self):
        """测试调拨多品多个bom库存"""
        origin_inventory = [('63203684930A01', 2), ('63203684930A02', 4), ('63203684930A02', 6), ('63203684930B01', 1),
                            ('63203684930B02', 5)]
        transfer_demand_goods_list = [('63203684930', 3)]
        sale_sku = '63203684930'
        IMSDBOperator.delete_qualified_inventory([sale_sku])
    ...
    def test_10_trans_single_goods_with_components(self):
        ...
        IMSDBOperator.delete_qualified_inventory([sale_sku])
        trans_out_sj_kw_ids = [
            wms_logics.base_get_kw(1, 5, len(origin_inventory), self.trans_out_id, self.trans_out_to_id)]
