...
class OmsOrderFollowTrace(BaseModel):
    ...
    id = BigAutoField()
    item_sku_code = CharField()
    max_eta_days = IntegerField(column_name='maxEtaDays', null=True)
    max_product_eta_days = IntegerField(column_name='maxProductEtaDays', null=True)
    min_eta_days = IntegerField(column_name='minEtaDays', null=True)
    min_expected_delivery_eta = DateField(column_name='minExpectedDeliveryEta', null=True)
    min_product_eta = DateField(column_name='minProductEta', null=True)
    min_product_eta_days = IntegerField(column_name='minProductEtaDays', null=True)
    order_type = IntegerField(null=True)
    pay_time = DateTimeField(null=True)
...
class OmsOrderFollowTraceDetail(BaseModel):
    ...
    id = BigAutoField()
    in_warehouse_eta = DateField(column_name='inWarehouseEta', null=True)
    in_warehouse_max_eta = DateField(column_name='inWarehouseMaxEta', null=True)
    in_warehouse_min_eta = DateField(column_name='inWarehouseMinEta', null=True)
    intercept_status = IntegerField(constraints=[SQL("DEFAULT 0")], null=True)
    item_sku_code = CharField()
    labor_status = IntegerField(constraints=[SQL("DEFAULT 0")], null=True)
    latest_state = IntegerField(constraints=[SQL("DEFAULT 1")], null=True)
    logistics_actual = DecimalField(column_name='logisticsActual', null=True)
    logistics_max = IntegerField(column_name='logisticsMax', null=True)
    logistics_min = IntegerField(column_name='logisticsMin', null=True)
    max_eta = DateField()
    min_eta = DateField()
