...
XMIND_FILE_PATH = '/Users/linzhongjie/Documents/爆米/测试用例/国际化.xmind'
...
def parse_case(parent_node, child, case_data):
    ...
    case_detail = {
        ...
        "step": child["topics"][0].get("title", ""),  # 步骤描述，暂时写死用例标题的下一级
        "expect": child["topics"][0]["topics"][0].get("title", "")  # 预期结果，暂时写死用例步骤的下一级
    }
    # TODO 支持 用例类型、前置条件、备注 的解析
...
def write_excel(sheet_name: str, cases: list):
    ...
    for case in cases:
        ...
        row_data.extend(module_list[0:3])
    ...
    work_book.save(f"{OUTPUT_FILE_PATH}{sheet_name}.xlsx")
